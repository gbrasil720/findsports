...
writeFileSync(
	...
	[
		...
		`  const getH = typeof h.get === 'function' ? (k) => h.get(k) : (k) => h[k] ?? null`,
		`  const host = getH('x-forwarded-host') || getH('host') || 'localhost'`,
		`  const proto = getH('x-forwarded-proto') || 'https'`,
		...
		`  })`,
		`  return server.fetch(fetchRequest)`,
		`}`,
	].join("\n"),
);
