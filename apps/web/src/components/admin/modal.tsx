...
export function Modal({ title, children, open, onClose }: Props) {
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="onside-dialog max-h-[min(90dvh,900px)] min-h-0 max-w-[calc(100vw-2rem)] overflow-hidden overscroll-contain rounded-none border-[1.5px] border-[var(--onside-ink)] bg-[var(--onside-paper)] p-0 text-[var(--onside-ink)] shadow-[8px_8px_0_var(--onside-ink)] sm:max-w-lg">
