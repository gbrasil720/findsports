...
const SIGNIN_PATH = '/sign-in/email'
...
export function ehLoginEmail(url: string) {
  let path: string
  try {
    path = new URL(url).pathname
  } catch {
  ...
  return path.toLowerCase().replace(/\/+$/, '').endsWith(SIGNIN_PATH)
}
...
export function decidirEntrada(input: {
  ...
  const fechado = input.fechadoEmRuntime
...
export async function admitirConta(email: string): Promise<void> {
